...
class PlaneProcessor():
    ...
    def factory(method, **kwargs):
        if method == "3_points_plane":
            return PlaneTransformer(**kwargs)
        elif method == "ransac":
            return PlaneRemover(**kwargs)
        else:
            ValueError(method)
...
class PlaneRemover():
    ...
    def apply(self, data):
        ...
        [a, b, c, d] = np.append(self.normal, self.intercept)

        # remove ground
        denominator = np.sqrt(a**2 + b**2 + c**2)
        data = np.hstack((data, np.ones(data.shape[0])[:, None]))  # Add 1 as fourth dimension to all points
        dist = np.abs(np.dot(data, np.array([a, b, c, d]))) / denominator
        data = data[dist > 0.2][:, :3]
