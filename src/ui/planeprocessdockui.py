...
class PlaneProcessDock(QtWidgets.QDockWidget):
    def __init__(self, parent=None):
        ...
        # toggle button here
        self.enablePlaneProcess = QtWidgets.QCheckBox("Enable coordinate transformation")
        self.mainLayoutV.addWidget(self.enablePlaneProcess)
        ...
        # table
        self.planeProcessHeader = QtWidgets.QLabel("")
        self.planeProcessHeader.setEnabled(False)
        self.planeProcessResult = QtWidgets.QLabel("")
        self.planeProcessResult.setEnabled(False)
        self.updateResult(None)
        self.groupLayoutV.addWidget(self.planeProcessHeader)
        self.groupLayoutV.addWidget(self.planeProcessResult)
    ...
    def set_from_config(self, **kwargs):
        self.enablePlaneProcess.setChecked(True)
        self.updateResult(kwargs["normal"], kwargs["intercept"])
        self.enableResults(True)

    def reset(self):
        self.enablePlaneProcess.setChecked(False)
        self.updateResult(None)
        self.enableResults(False)

    def connectOwnButtons(self):
        self.enablePlaneProcess.stateChanged.connect(self.togglePlaneProcess)

    def togglePlaneProcess(self):
        if self.enablePlaneProcess.isChecked():
    ...
    def updateResult(self, normal=None, intercept=None):
        ...
        if normal is None or intercept is None:
            self.planeProcessHeader.setText("Plane is not estimated")
            txt = ""
        else:
            self.planeProcessHeader.setText("Plane is estimated:")
            txt = self._get_result_text(normal, intercept)
        self.planeProcessResult.setText(txt)

    def enableResults(self, state):
        self.planeProcessHeader.setEnabled(state)
        self.planeProcessResult.setEnabled(state)
...
if __name__ == '__main__':
    ...
    dock = PlaneProcessDock(parent=win)
